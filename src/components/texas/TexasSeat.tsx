...
const TexasSeat: React.FC<TexasSeatProps> = ({
    player,
    isActive,
    isWinner,
    phase,
    ...
    const cardsSection = (
        ...
        >
            {player.cards.map((c, i) => {
                // Texas visibility: Hidden unless it's Me OR phase is Showdown/Result
                const isHidden = !isMe && phase !== GamePhase.SHOWDOWN && phase !== GamePhase.RESULT;
                return (
                    <CardUI
                        key={i}
                        card={c}
                        hidden={isHidden}
                        className={`deal-card ${i > 0 ? '-ml-6 md:-ml-8' : ''}`}
                        style={{ zIndex: i + 1 }}
                    />
                );
            })}
